...
# Read file
#with open(0) as data_file:
#with open('Sample Input 2.txt') as data_file:
#    data = data_file.read().splitlines()

#for line in fileinput.input('Sample Input 2.txt'):
#    data = line.splitlines()

f = open('Sample Input 2.txt')
data = f.readlines()
...
for c in range(len(conf_matrix_norm)):
    for r in range(len(conf_matrix_norm[c])):
        if conf_matrix_norm[r][c] == 0:
            ...
            I_x_y[r][c] = conf_matrix_norm[r][c] * math.log10(tmp)
...
# Denominator
H_x = [-1 * i * math.log10(i) for i in row_sum]
H_x = sum(H_x)

col_sum = [i for i in col_sum if i != 0]
H_y = [-1 * i * math.log10(i) for i in col_sum]
H_y = sum(H_y)
